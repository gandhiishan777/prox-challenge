...
and high findings are fixed (see PHASE_LOG "Phase 6"). Of the findings below,
**1, 2, 5, 8 and 10 have since been fixed too** (PHASE_LOG "Phase 7") — the
cheap-and-safe set. The rows are kept with their original text so the review
trail stays readable; the still-open ones are 3, 4, 6, 7 and 9.

| # | Issue | Why it matters | Status |
|---|---|---|---|
| 1 | `parseThickness` reads `"10mm"` as 10 **inches** | ~25× error in a capability check ("MIG is rated 22 ga to 3/8\", compare against the 10\" you described") | **Fixed** — unit-aware branch converts mm/cm to inches and labels both; regression tests. |
| 2 | `"1 1/2 inch"` parses as `0.5"`; `"3/8 galvanized"` parses as `"8 Ga"` | Wrong thickness silently accepted | **Fixed** — compound fractions summed, gauge regex word-bounded; regression tests. |
| 3 | SVG and mermaid artifacts render in the **parent origin**, not the sandbox | Two of six artifact types bypass the iframe entirely; DOMPurify and mermaid `securityLevel: "strict"` are the only barrier | Open. Architectural change; the mitigations are real but the asymmetry is undocumented outside this table. |
| 4 | `permissionMode: "bypassPermissions"` makes a hand-maintained denylist the only barrier | A future SDK built-in not on the list would run unprompted | Open. A `canUseTool` allow-list callback is the right fix; the denylist was the fast one after `repl` appeared with no root cause found. |
| 5 | Several troubleshooting entries drop printed causes | `no-arc-ignition` has one placeholder where om-43 prints four; `no-power` drops "tripped thermal protection" with its cross-link to the duty-cycle page — the manual's best cross-reference | **Fixed** — om-43/om-44 causes curated in full (thermal trip with the p. 19 cross-reference, reset button, contact-tip causes; gas cause scoped to TIG only). Both knowledge gates re-run green. |
| 6 | Eval parses complete messages, not stream deltas | It never exercises chunk boundaries — the class of bug that produced two real parser defects | Open. The unit suite covers boundaries directly; the eval would need to consume SSE. |
| 7 | Eval gates prove a tool was *called*, not that its result was used | A model could call the lookup and still state a fabricated number | Open. Only the hand-written `must_not_contain` traps catch this today. |
| 8 | `require` shim uses `name in scope`, so `require("constructor")` resolves | Contradicts the README's "an import we do not stock cannot resolve"; not an escape, since `new Function` already grants `globalThis` inside the frame | **Fixed** — own-property check; test added. |
| 9 | An artifact containing `</antArtifact>` truncates but reports `complete: true` | Truncation is defensible and matches claude.ai; presenting it as clean is not | Open. The panel's "incomplete" badge exists but is not triggered. |
| 10 | No Python dependency manifest for the pipeline | Undercuts "reproducible"; `fitz`, `PIL`, `dotenv`, `anthropic` are assumed present | **Fixed** — `scripts/requirements.txt`. |
...
Across the whole repo: **87/87 unit tests**, `npx tsc --noEmit` clean, production
...
npm test                  # 87 unit tests, no API key needed, ~0.5s
